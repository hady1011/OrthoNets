...
from model.OrthoNet import orthonet18, orthonet34, orthonet50, orthonet101, orthonet152
from model.OrthoNet_MOD import orthonet_mod_18, orthonet_mod_34, orthonet_mod_50, orthonet_mod_101, orthonet_mod_152
...
def parse():
    ...
    model_names = sorted(name for name in models.__dict__
                     ...
                     and callable(models.__dict__[name])) + ['orthonet_18', 'orthonet_34', 'orthonet_50', 'orthonet_101', 'orthonet_152'
                                                             , 'orthonet_mod_18', 'orthonet_mod_34', 'orthonet_mod_50', 'orthonet_mod_101', 'orthonet_mod_152', "fcanet_50"]

    parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')
    parser.add_argument('data', metavar='DIR', nargs='*', default=['DATA/ImgNet/train/'],
                        help='path(s) to dataset (if one path is provided, it is assumed\n' +
                       'to have subdirectories named "train" and "val"; alternatively,\n' +
                       'train and val paths can be specified directly by providing both paths as arguments)')
    parser.add_argument('--arch', '-a', metavar='ARCH', default='orthonet_mod_50',
                        choices=model_names,
    ...
    parser.add_argument('--numclasses', default=1000, type=int, metavar='N',
                        help='number of total epochs to run')
    ...
    parser.add_argument('--evaluate_model', type=str, default='pretrained_models/orthonet_mod_50_imagenet.pth.tar',
                        help='the model for evaluation')
    parser.add_argument('--dataset_location', type=str, default='/ImgNet/',
                        help='the model for evaluation')
...
def main():
    ...
    if args.pretrained:
        ...
        if args.arch == 'fcanet_34':
            model = fcanet34(pretrained=True)
        elif args.arch == 'fcanet_50':
            model = fcanet50(pretrained=True)
        elif args.arch == 'fcanet_101':
            model = fcanet101(pretrained=True)
        elif args.arch == 'fcanet_152':
        ...
        if args.arch == 'fcanet_34':
            model = fcanet34(args.numclasses)
        elif args.arch == 'fcanet_50':
            model = fcanet50(args.numclasses)
            print('i created the model')
        elif args.arch == 'fcanet_101':
            model = fcanet101(args.numclasses)
        elif args.arch == 'fcanet_152':
            model = fcanet152(args.numclasses)
        elif args.arch == 'orthonet_18':
            model = orthonet18(args.numclasses)
        elif args.arch == 'orthonet_34':
            model = orthonet34(args.numclasses)
        elif args.arch == 'orthonet_50':
            model = orthonet50(args.numclasses)
        elif args.arch == 'orthonet_101':
            model = orthonet101(args.numclasses)
        elif args.arch == 'orthonet_152':
            model = orthonet152(args.numclasses)
        elif args.arch == 'orthonet_mod_18':
            model = orthonet_mod_18(args.numclasses)
        elif args.arch == 'orthonet_mod_34':
            model = orthonet_mod_34(args.numclasses)
        elif args.arch == 'orthonet_mod_50':
            model = orthonet_mod_50(args.numclasses)
        elif args.arch == 'orthonet_mod_101':
            model = orthonet_mod_101(args.numclasses)
        elif args.arch == 'orthonet_mod_152':
            model = orthonet_mod_152(args.numclasses)
        else:
    ...
    if args.resume:
        ...
        def resume():
            if os.path.isfile(args.resume):
                ...
                dist_print("=> loaded checkpoint '{}' (epoch {})".format(args.resume, checkpoint['epoch']))
            else:
    ...
    if args.evaluate:
        ...
        if 'state_dict' in checkpoint.keys():
            ...
            if using_one_gpu: state_dict = remove_model_from_state_dict(state_dict)
        else:
            state_dict = {}
            for k,v in checkpoint.items():
                prefix = "module." if not using_one_gpu else ""
    ...
    if len(args.data) == 1:
        ...
        traindir = args.dataset_location+'train/'
        valdir = args.dataset_location+'val/'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.local_rank == 0:
            ...
            if is_best and args.model_save_name is not None:
                ...
                save_checkpoint({
                    ...
                    'best_prec1': best_prec1,
                    'optimizer': optimizer.state_dict(),
                }, False, work_dir = "trained_models", filename=args.model_save_name)
...
def remove_model_from_state_dict(state_dict): 
    out = {}
    for key, value in state_dict.items(): out[key[7:]] = value
    return out
...
def train(train_loader, model, criterion, optimizer, epoch, logger, scheduler):
    ...
    for i, data in enumerate(train_loader):
        ...
        if i%args.print_freq == 0:
            ...
            rem_time = (100 - epoch) * (train_loader_len * batch_time.val // 60) + (
                        train_loader_len - i) * batch_time.val // 60
            if args.local_rank == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                      'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                      'RemTime {rem_day:.0f}d:{rem_hrs:.0f}h:{rem_min:.0f}m\t'
                      'Speed {3:.3f} ({4:.3f})\t'
                      ...
                      'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                       ...
                       batch_time=batch_time, rem_day=rem_time // 1440, rem_hrs=(rem_time // 60) % 24,
                       rem_min=rem_time % 60,
